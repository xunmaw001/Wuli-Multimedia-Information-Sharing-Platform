		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"86%","margin":"0 auto 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"20px","background":"rgba(162, 153, 136, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('siliaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"20px","background":"rgba(162, 153, 136, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('siliaoxinxi','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					label="私聊日期">
						<template slot-scope="scope">
							{{scope.row.siliaoriqi}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false' prop="shhf" label="回复"></el-table-column>
					<el-table-column :resizable='true' :sortable='false' v-if="isAuth('siliaoxinxi','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #AAAAAA","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"#AAAAAA","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('siliaoxinxi','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid #AAAAAA","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"#AAAAAA","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('siliaoxinxi','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"1px solid #AAAAAA","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"#AAAAAA","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('siliaoxinxi','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          zuopinfengmian: row.zuopinfengmian,
          zhanghao: row.zhanghao,
          xingming: row.xingming,
          siliaoneirong: row.siliaoneirong,
          siliaozhanghao: row.siliaozhanghao,
          siliaoxingming: row.siliaoxingming,
          siliaoriqi: row.siliaoriqi,
          shhf: row.shhf,
          id: row.id
        }
      }
